...
function UnPaidApplicationsTable({ leads }) {
  ...
  const {loading:leadStatusUpdateLoading,error:leadStatusUpdateError} = useSelector((state)=>state.leads)
  const [
  ...
  useEffect(() => {
    if (leadStatusUpdateError || statusUpdateError) {
      ...
      setToastMessage(leadStatusUpdateError?.message || statusUpdateError.message);
...
export default UnPaidApplicationsTable;
