function NavigationItem({ name, icon, isActive, onClick, showLabel = true }) {
  return (
    ...
    >
      ...
      {!showLabel && (
        <div className={`absolute top-full left-1/2 transform -translate-x-1/4 ${isActive ? 'mt-2' : 'mt-0'} px-2 py-1 bg-[#EBEBEB] text-[#214768] text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-50 pointer-events-none`}>
          {name}
